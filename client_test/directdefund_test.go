...
func TestDirectDefund(t *testing.T) {
	...
	chainI, err := chainservice.NewSimulatedBackendChainService(sim, bindings.Adjudicator.Contract, bindings.Adjudicator.Address, ethAccounts[1])
	if err != nil {
		t.Fatal(err)
	}
	chainB, err := chainservice.NewSimulatedBackendChainService(sim, bindings.Adjudicator.Contract, bindings.Adjudicator.Address, ethAccounts[2])
	if err != nil {
		t.Fatal(err)
	}
