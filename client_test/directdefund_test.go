...
func TestDirectDefund(t *testing.T) {
	...
	chainI, err := chainservice.NewSimulatedBackendChainService(sim, bindings.Adjudicator.Contract, bindings.Adjudicator.Address, ethAccounts[1], logDestination)
	if err != nil {
		t.Fatal(err)
	}
	chainB, err := chainservice.NewSimulatedBackendChainService(sim, bindings.Adjudicator.Contract, bindings.Adjudicator.Address, ethAccounts[2], logDestination)
	if err != nil {
		t.Fatal(err)
	}
